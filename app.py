...
def fetch_single(symbol: str):
    try:
        ...
        if len(hist) >= 2:
            ...
            return {"pct": round((curr - prev) / prev * 100, 2), "price": round(curr, 2)}
    except Exception:
        pass
    return None
...
def fetch_performance(sectors: dict) -> pd.DataFrame:
    ...
    with ThreadPoolExecutor(max_workers=len(items)) as ex:
        ...
        for f in as_completed(futures):
            ...
            data = f.result()
            if data:
                results.append({"セクター": name, "変化率": data["pct"], "現在値": data["price"], "ティッカー": sym})
    if not results:
        return pd.DataFrame()
    return pd.DataFrame(results).sort_values("変化率", ascending=True).reset_index(drop=True)
...
def fetch_semicon_heatmap() -> pd.DataFrame:
    ...
    for group, subsector, tickers in SEMICON_SUBSECTORS:
        ...
        if perfs:
            ...
            rows.append({"group": group, "subsector": subsector, "pct": avg, "size": max(abs(avg), 0.15)})
...
def render_sector_view(sectors: dict, market_name: str):
    ...
    fig = px.bar(df, x="変化率", y="セクター", orientation="h", text="text", color="変化率",
                 color_continuous_scale=[[0.0, "#ef5350"], [0.5, "#f5f5f5"], [1.0, "#26a69a"]],
                 color_continuous_midpoint=0, title=f"{market_name}セクター 前日比パフォーマンス")
    fig.update_traces(textposition="outside", marker_line_width=0)
    fig.update_layout(height=max(420, len(df) * 44), coloraxis_showscale=False,
                      xaxis_title="前日比 (%)", yaxis_title="", showlegend=False,
                      plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                      font=dict(size=13), margin=dict(l=10, r=110, t=50, b=10))
...
def render_semicon_heatmap():
    with st.spinner("半導体・AI詳細データを取得中..."):
    ...
    fig = px.treemap(df, path=["group", "subsector"], values="size", color="pct",
                     color_continuous_scale=[[0.00, "#6b0000"], [0.30, "#2d0a0a"], [0.45, "#141f14"],
    ...
    fig.update_traces(texttemplate="<b>%{label}</b><br>%{customdata[0]:+.2f}%",
                      ...
                      marker_line_color="rgba(0,0,0,0.6)", root_color="rgba(20,20,20,1)")
    fig.update_layout(height=720, paper_bgcolor="rgba(14,14,14,1)",
                      margin=dict(t=10, l=10, r=10, b=10),
                      coloraxis_showscale=False, font=dict(color="#ffffff"))
    st.plotly_chart(fig, use_container_width=True)
    with st.expander("📋 詳細テーブルを表示"):
        tbl = (df[["group", "subsector", "pct"]]
               .rename(columns={"group": "グループ", "subsector": "サブセクター", "pct": "前日比(%)"})
               .sort_values("前日比(%)", ascending=False).reset_index(drop=True))
...
tab_us, tab_jp, tab_semi = st.tabs(["🇺🇸 米国 (S&P500セクターETF)", "🇯🇵 日本 (TOPIX-17 ETF)", "🔬 半導体・AI 詳細ヒートマップ"])
...
st.divider()
st.caption("データ: Yahoo Finance ｜ 米国: SPDR セクターETF ｜ 日本: NEXT FUNDS TOPIX-17 ETF ｜ 半導体: 代表銘柄の平均前日比")国: SPDR セクターETF ｜ 日本: NEXT FUNDS TOPIX-17 ETF ｜ 半導体: 代表銘柄の平均前日比")
