...
SEMICON_SUBSECTORS = [
    # ── AI・データセンター ──────────────────────────────────────────────────
    ("AI・データセンター",    "GPU / AIアクセラレータ",               ["NVDA", "AMD", "INTC"]),
    ("AI・データセンター",    "AIサーバー / HPC",                     ["SMCI", "DELL", "HPE", "NVDA"]),
    ("AI・データセンター",    "CPU / DPU",                            ["AMD", "INTC", "ARM"]),
    ("AI・データセンター",    "ネットワークIC / SmartNIC",            ["MRVL", "AVGO", "INTC"]),
    ("AI・データセンター",    "クラウドAI / ハイパースケーラー",      ["MSFT", "GOOGL", "AMZN", "META"]),
    ("AI・データセンター",    "エッジAI SoC",                         ["QCOM", "AMD", "6723.T", "6963.T"]),
    ("AI・データセンター",    "車載AI / ADAS",                        ["QCOM", "MCHP", "6902.T", "6723.T"]),
    ("AI・データセンター",    "ロボット / 産業AI",                    ["NVDA", "ISRG", "6954.T", "6273.T"]),
    ("AI・データセンター",    "熱交換器 / 液冷・空冷",                ["SMCI", "VRT", "6361.T", "6367.T"]),
    ("AI・データセンター",    "データセンター電力・UPS",              ["EATON", "VRT", "PWR"]),
    # ── ファブレス・設計 ────────────────────────────────────────────────────
    ("ファブレス・設計",      "通信 / モバイルSoC",                   ["QCOM", "MRVL", "AVGO"]),
    ("ファブレス・設計",      "FPGA / プログラマブル",                ["AMD", "LSCC", "MCHP"]),
    ("ファブレス・設計",      "ネットワークスイッチ・ルーターIC",     ["AVGO", "MRVL", "ANET"]),
    ("ファブレス・設計",      "グラフィクス / ゲーミングGPU",         ["NVDA", "AMD"]),
    ("ファブレス・設計",      "セキュリティ / 暗号化チップ",          ["MCHP", "ATML", "SLAB"]),
    # ── アナログ・電源IC ────────────────────────────────────────────────────
    ("アナログ・電源IC",      "汎用アナログ / DAC / ADC",             ["TXN", "ADI", "MCHP"]),
    ("アナログ・電源IC",      "電源管理IC (PMIC)",                    ["MPWR", "TXN", "ADI", "ON"]),
    ("アナログ・電源IC",      "モータードライバ / モーション制御",    ["TXN", "ADI", "ON"]),
    ("アナログ・電源IC",      "センサー / MEMS",                      ["ADI", "TXN", "STM"]),
    ("アナログ・電源IC",      "マイコン / エンベデッド",              ["MCHP", "TXN", "STM", "SLAB"]),
    # ── 通信・RF半導体 ──────────────────────────────────────────────────────
    ("通信・RF半導体",        "RF / フロントエンドモジュール",        ["QCOM", "SWKS", "QRVO", "AVGO"]),
    ("通信・RF半導体",        "5G / ミリ波",                          ["QCOM", "MRVL", "AVGO"]),
    ("通信・RF半導体",        "Wi-Fi / Bluetooth SoC",                ["QCOM", "SWKS", "MRVL"]),
    ("通信・RF半導体",        "光トランシーバー / CMOS フォトニクス", ["COHR", "LITE", "VIAV", "5802.T"]),
    ("通信・RF半導体",        "レーザー / フォトダイオード",          ["COHR", "LITE", "6965.T", "6976.T"]),
    # ── SiC・GaN・次世代パワー ──────────────────────────────────────────────
    ("SiC・GaN・次世代パワー","SiC パワー半導体",                     ["WOLF", "ON", "STM"]),
    ("SiC・GaN・次世代パワー","GaN パワー半導体",                     ["WOLF", "MTSI", "PI"]),
    ("SiC・GaN・次世代パワー","EV向けインバータ・コンバータ",         ["ON", "STM", "WOLF", "6504.T"]),
    ("SiC・GaN・次世代パワー","パワーモジュール",                     ["ON", "AEHR", "6503.T", "6723.T"]),
    # ── ファウンドリ・OSAT ──────────────────────────────────────────────────
    ("ファウンドリ・OSAT",    "純粋受託製造 (ピュアプレイ)",          ["TSM", "GFS", "UMC"]),
    ("ファウンドリ・OSAT",    "後工程・パッケージング (OSAT)",        ["AMKR", "ASX", "4062.T"]),
    ("ファウンドリ・OSAT",    "先端パッケージ CoWoS / SoIC",          ["TSM", "AMKR", "4062.T", "6967.T"]),
    ("ファウンドリ・OSAT",    "シリコンウェハ",                       ["SUMCO", "3436.T", "4063.T"]),
    # ── 半導体製造装置 ──────────────────────────────────────────────────────
    ("半導体製造装置",        "成膜 / CVD / PVD",                     ["AMAT", "8035.T", "6590.T"]),
    ("半導体製造装置",        "エッチング",                           ["LRCX", "AMAT", "8035.T", "6728.T"]),
    ("半導体製造装置",        "露光装置 / EUV",                       ["ASML", "7731.T", "7751.T"]),
    ("半導体製造装置",        "計測 / 検査",                          ["KLAC", "ONTO", "6920.T", "6857.T"]),
    ("半導体製造装置",        "洗浄 / CMP",                           ["AMAT", "LRCX", "7735.T"]),
    ("半導体製造装置",        "イオン注入 / 熱処理",                  ["ACLS", "AMAT", "6590.T"]),
    ("半導体製造装置",        "後工程 / ダイシング / ボンディング",   ["BESI", "6146.T", "6856.T"]),
    # ── EDA・半導体IP ────────────────────────────────────────────────────────
    ("EDA・半導体IP",         "EDA設計ツール",                        ["SNPS", "CDNS"]),
    ("EDA・半導体IP",         "半導体IP / アーキテクチャ",            ["ARM", "SNPS", "CDNS"]),
    ("EDA・半導体IP",         "テスト / シミュレーション",            ["SNPS", "KEYS", "AEIS"]),
    # ── メモリ・ストレージ ──────────────────────────────────────────────────
    ("メモリ・ストレージ",    "DRAM / HBM",                           ["MU", "3436.T"]),
    ("メモリ・ストレージ",    "NAND / エンタープライズSSD",           ["WDC", "MU", "285A.T"]),
    ("メモリ・ストレージ",    "エンタープライズストレージ",           ["NTAP", "PSTG", "STX"]),
    ("メモリ・ストレージ",    "SSDコントローラ",                      ["MRVL", "MU"]),
    # ── 半導体材料 ──────────────────────────────────────────────────────────
    ("半導体材料",            "特殊ガス / 高純度薬液",                ["ENTG", "CMC", "4091.T", "4109.T"]),
    ("半導体材料",            "フォトレジスト / 露光材料",            ["ENTG", "4063.T", "4186.T"]),
    ("半導体材料",            "CMP スラリー / パッド",                ["ENTG", "CMC", "7735.T"]),
    ("半導体材料",            "フォトマスク / ブランクス",            ["PHOTON", "7741.T", "5201.T"]),
    ("半導体材料",            "ABF基板 / ガラス基板",                 ["4062.T", "6967.T", "5201.T", "5214.T"]),
    # ── 電子部品 (日本中心) ─────────────────────────────────────────────────
    ("電子部品",              "MLCC / 積層セラコン",                  ["6981.T", "6762.T", "6976.T"]),
    ("電子部品",              "パワーインダクタ",                     ["6762.T", "6981.T", "6773.T"]),
    ("電子部品",              "水晶デバイス / クロック",              ["6779.T", "6728.T"]),
    ("電子部品",              "コネクタ",                             ["APH", "TEL", "6752.T"]),
]
